...
**✨ 주요 특징:**
- 🔒 **연락처 수집**: 진단 결과 확인 전 이메일 또는 전화번호 입력 필수
- 📊 **관리자 패널**: 진단 이력 조회, 필터링, 통계, CSV 다운로드
- 🔐 **관리자 인증**: 보안된 관리자 로그인 시스템
...
- ✅ **연락처 수집**: 진단 결과 확인 전 이메일/전화번호 입력 (마케팅 활용)
- ✅ **관리자 패널**: 진단 이력 조회, 필터링, 통계 분석, CSV 다운로드
...
### 페이지
- `/` - 메인 재무진단 페이지 (천단위 콤마 입력, **연락처 수집 기능**)
- `/standards` - 신보 업종별 표준지표 조회 (필터링 기능)
- `/formulas` - 재무비율 계산식 안내 (12개 지표)
- `/test` - 숫자 입력 테스트 페이지
- `/admin/login` - **관리자 로그인** (ID: admin, PW: admin123)
- `/admin/dashboard` - **관리자 대시보드** (진단 이력 관리)
...
- **입력 팁**: 숫자 입력 시 자동으로 천단위 콤마가 표시됩니다
...
#### 3️⃣ 연락처 입력
- **이메일 또는 전화번호** 중 하나 필수 입력
- 입력 후에만 진단 결과 확인 가능
- 연락처는 마케팅 및 후속 컨설팅 목적으로 활용됩니다

**입력 형식:**
- 이메일: `example@domain.com` (영문, 숫자, ., _, - 허용)
- 전화번호: 
  - 휴대폰: `010-1234-5678`, `01012345678` (010, 011, 016, 017, 018, 019)
  - 일반전화: `02-123-4567`, `031-1234-5678` (02, 031~070)

**허용되는 형식:**
- 이메일: example@domain.com, user@naver.com 등
- 휴대폰: 010-1234-5678, 01012345678, 010 1234 5678 등
- 일반전화: 02-123-4567, 031-123-4567, 051-123-4567 등
...
#### 1️⃣ 로그인
- URL: `/admin/login`
- 기본 계정: **admin** / **admin123** (운영 시 반드시 변경!)

#### 2️⃣ 대시보드 기능
- **통계 현황**: 총 진단 건수, 오늘 진단, 리스크별 분포
- **진단 이력 조회**: 전체 진단 기록 확인
- **필터링**: 날짜, 업종, 연락처로 검색
- **상세 보기**: 개별 진단 결과 상세 정보
- **CSV 다운로드**: 전체 데이터 엑셀 다운로드
